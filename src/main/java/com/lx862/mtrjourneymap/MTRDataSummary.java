...
import com.lx862.mtrjourneymap.wrapper.impl.MTRRouteImpl;
import com.lx862.mtrjourneymap.wrapper.MTRRoutePlatform;
import com.lx862.mtrjourneymap.wrapper.impl.MTRSimplifiedRouteImpl;
...
public class MTRDataSummary {
        ...
        for (Station station : new ArrayList<>(dataInstance.stations)) {
            ...
            for (MTRRoute route : routePassing) {
                ...
                if (basicRouteInfos.contains(basicRouteInfo))
                    continue;
                if (!MTRSurveyorConfig.INSTANCE.visibility.showHiddenRoute && route.isHidden())
                    continue;
    ...
    public static MTRDataSummary of(MinecraftClientData data) {
        return new MTRDataSummary(data, data.simplifiedRouteIdMap.values().stream().map(MTRSimplifiedRouteImpl::new)
